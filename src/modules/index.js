                    ...
                    while (unvisitedPoints.length >= 1 && foundEndPoint === false) {
                        ...
                        for(let i=0; i < searchPoints.length ; i++) {
                            ...
                            if (currentPoint.index === endPoint) {
                                ...
                                const left = `${y}-${x - 1}`;
                                const right = `${y}-${x + 1}`;
                                const neighbors = [left, up, down, right].filter(point => boardCoordinateCopy[point] !== undefined && boardCoordinateCopy[point].pointType === 'unvisited');
                                if (neighbors.length > 0) {
                                    neighbors.forEach(point => {
                                        if (boardCoordinateCopy[point].shortest > boardCoordinateCopy[point].cost + currentPoint.shortest) {
